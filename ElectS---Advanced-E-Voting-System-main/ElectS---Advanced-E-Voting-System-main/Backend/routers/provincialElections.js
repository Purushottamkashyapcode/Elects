...
router.get('/', async (req, res) => {
    try {
        const elections = await ProvincialElection.find();
...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        const electionStartTime = createFullDate(electionDate, startTime);
...
// Update Provincial Election by ID
...
// Apply for Provincial Election (add candidate)
...
// Endpoint to vote for candidate in Provincial Election
...
router.delete('/:id', (req, res) => {
    Service.deleteById(req, res, ProvincialElection, name).catch((error) => {
        res.status(500).send(error + " Server Error");
    });
});
